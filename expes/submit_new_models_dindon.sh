#!/bin/bash
# Submit HybridGTTFN + fixed CrossAttentionTFN to dindon for the full 21-dataset benchmark
set -e
...
$SCP_CMD ../models.py $REMOTE:$REMOTE_DIR/models.py
$SCP_CMD ../gt_improvements.py $REMOTE:$REMOTE_DIR/gt_improvements.py
...
MODELS="CrossAttentionTensorFieldNetwork HybridGTTFN"
...
MAP_LOCAL=$(mktemp /tmp/param_map_new.XXXXXX)
...
$SCP_CMD "$MAP_LOCAL" $REMOTE:$REMOTE_DIR/expes/results/enhanced/param_map_new_models.txt
...
echo "Total new-model jobs: $N_JOBS"

echo "=== Generating SLURM array script (local) ==="
SLURM_LOCAL=$(mktemp /tmp/submit_new_array.XXXXXX)
cat > "$SLURM_LOCAL" << 'SLURM'
#!/bin/bash
#SBATCH --job-name=tfn_new
#SBATCH --output=slurm_logs/new_%A_%a.out
#SBATCH --error=slurm_logs/new_%A_%a.err
...
MAPFILE="results/enhanced/param_map_new_models.txt"
...
$SCP_CMD "$SLURM_LOCAL" $REMOTE:$REMOTE_DIR/expes/submit_new_models_array.sh
rm -f "$SLURM_LOCAL"
...
while [ $BATCH_START -lt $N_JOBS ]; do
    ...
    $SSH_CMD "cd $REMOTE_DIR/expes && sed 's/#SBATCH --array=0-$((N_JOBS-1))/#SBATCH --array=$BATCH_START-$BATCH_END/' submit_new_models_array.sh | sbatch"
