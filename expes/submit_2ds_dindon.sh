#!/bin/bash
# Submit jobs for TwoLeadECG and UMD to dindon
set -e
...
MAP_LOCAL=$(mktemp /tmp/param_map_2ds.XXXXXX)
python3 - "$MAP_LOCAL" <<'PY'
import sys
out = sys.argv[1]
ds_list = ["TwoLeadECG", "UMD"]
models = ["GTTensorFieldNetworkV2", "CrossAttentionTensorFieldNetwork", "HybridGTTFN"]
fracs = [10, 20, 30, 50, 70, 100]
trials = [0, 1, 2, 3]
lines = []
i = 0
...
$SCP_CMD "$MAP_LOCAL" $REMOTE:$REMOTE_DIR/expes/results/enhanced/param_map_2ds.txt

N_JOBS=$(wc -l < "$MAP_LOCAL" | tr -d '[:space:]')
echo "Total jobs: $N_JOBS"

echo "=== Generating SLURM array script ==="
SLURM_LOCAL=$(mktemp /tmp/submit_2ds_array.XXXXXX)
cat > "$SLURM_LOCAL" << 'SLURM'
#!/bin/bash
#SBATCH --job-name=tfni_2ds
#SBATCH --output=slurm_logs/2ds_%A_%a.out
#SBATCH --error=slurm_logs/2ds_%A_%a.err
#SBATCH --array=0-1000
...
OFFSET=${OFFSET:-0}
MAPFILE="results/enhanced/param_map_2ds.txt"
...
$SCP_CMD "$SLURM_LOCAL" $REMOTE:$REMOTE_DIR/expes/submit_2ds_array.sh
rm -f "$SLURM_LOCAL"

echo "=== Submitting ==="
if [ $N_JOBS -le 1001 ]; then
    LAST_IDX=$((N_JOBS - 1))
    echo "Submitting: array=0-$LAST_IDX ($N_JOBS jobs)"
    $SSH_CMD "cd $REMOTE_DIR/expes && sbatch --array=0-$LAST_IDX submit_2ds_array.sh"
else
    BATCH_START=0
    BATCH_NUM=0
    while [ $BATCH_START -lt $N_JOBS ]; do
        ...
        $SSH_CMD "cd $REMOTE_DIR/expes && OFFSET=$BATCH_START sbatch --array=0-$LAST_IDX submit_2ds_array.sh"
