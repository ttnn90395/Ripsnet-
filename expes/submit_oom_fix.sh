#!/bin/bash
#SBATCH --job-name=tfn_oom
#SBATCH --output=slurm_logs/oom_%A_%a.out
#SBATCH --error=slurm_logs/oom_%A_%a.err
#SBATCH --array=0-19
...
echo "=== OOM fix $SLURM_ARRAY_TASK_ID: $DS $MODEL ${PCT}% trial=$TRIAL clf=$CLF aug=$AUG ==="
echo "Node: $(hostname)"
echo "Time: $(date)"
