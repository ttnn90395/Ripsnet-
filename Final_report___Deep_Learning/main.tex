...
Table~\ref{tab:model_cost} summarizes the parameter counts and per-forward time complexity of each model family.
For the UCR datasets ($N=15$--$200$ points, $k=16$ neighbors, $L=1$--$3$ layers), all models train in seconds on CPU.
The TFN family pays $O(LNk)$ per forward pass; the distance-matrix model pays $O(N^2)$; PointNet pays $O(N)$.
The $O(N^2)$ cost of the distance-matrix model is not a bottleneck at these scales but would become one for point clouds with thousands of points.

\begin{table}[ht]
\centering
\caption{Parameter counts and per-forward time complexity for the UCR configurations ($N$ points, $k$ neighbors, $L$ layers, $T$ transformer layers). Typical $N$: 15--200; $k=16$; $L$: 1--3.}
...
\midrule
GTTensorFieldNetworkV2 & 2k--120k & $O(LNk)$ \\
TensorFieldNetwork & same as GTTFNv2 & $O(LNk)$ \\
OnEquivariantTFN & $2\times$ TFN & $O(2LNk)$ \\
AttentionTFN & 80k--300k & $O(LNk)$ \\
CrossAttentionTFN & 180k--400k & $O(LNk + TN^2D)$ \\
HybridGTTFN & $\sim$280k & $O(LNk + N^2)$ \\
StochasticTFN & 200k--350k & $O(LNk)$ \\
DistanceMatrixRaggedModel & 50k--110k & $O(N^2)$ \\
PersNet & $\sim$160k & $O(N)$ \\
PointNetTutorial & $\sim$155k & $O(N)$ \\
MultiInputModel & $\sim$350k & $O(N)$ \\
\bottomrule
\end{tabular}
\end{table}
