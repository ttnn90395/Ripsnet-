...
\caption{XGBoost accuracy vs.\ training fraction for each model family, averaged over 21 datasets (mean $\pm$ SEM). PersNet starts lowest at 20\% but reaches the highest accuracy at 100\%, while HybridGTTFN shows the most consistent improvement across fractions.}
...
\caption{CPU timing benchmarks on CBF (15 samples, 126 points each). \textbf{Left:} forward+backward time per pass. \textbf{Right:} parameter counts (log scale). The TFN family is 50--130$\times$ slower than PointNet, but all models train in under a second per epoch at this scale.}
\label{fig:timing}
\end{figure}
...
\caption{Mean test accuracy (21 datasets, best-config) vs.\ total CPU time per forward+backward pass. Bubble size is proportional to $\sqrt{\text{parameters}}$. The TFN variants achieve the highest accuracy but at 50--130$\times$ the cost of PointNet and DMR models, which remain competitive at a fraction of the compute.}
