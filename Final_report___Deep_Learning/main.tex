...
On a 21-dataset UCR benchmark, the best improved TFN variant (HybridGTTFN) achieves 79.2\% mean accuracy under a best-configuration protocol, competitive with classical baselines (DTW k-NN: 73.6\%) while maintaining exact rotation invariance.
However, at a fixed 100\% training fraction with a single classifier, the simpler distance-matrix and PointNet baselines perform comparably (73.1\% and 73.4\%), suggesting that configuration sensitivity rather than architecture is the primary lever for these models.
\end{abstract}
...
The gap between the two protocols (e.g., HybridGTTFN: 79.2\% best vs.\ 70.7\% fixed) reflects the sensitivity of these models to hyperparameter and configuration choices.
...
\textbf{GTTFNv2}$^{\ddagger}$ & 66.0 $\pm$ 15.3 \\
\textbf{CrossAttn}$^{\ddagger}$ & 66.3 $\pm$ 12.7 \\
\textbf{HybridGTTFN}$^{\ddagger}$ & 70.7 $\pm$ 14.4 \\
DMR & 73.1 $\pm$ 14.0 \\
PersNet & 73.4 $\pm$ 11.5 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Comparison with published baselines.}
Table~\ref{tab:sota} compares the best model in this study against published results from the time-series classification literature on the same 21 datasets.
MiniRocket \cite{minirocket2021} and ResNet \cite{resnet2019} achieve substantially higher accuracy (88.9\% and 87.3\% respectively) than any model in this study, including the best-configured HybridGTTFN (79.2\%).
This gap reflects the fact that time-series-specific architectures (convolutional, residual, kernel-based) are better tuned to the temporal structure of UCR data than geometric models designed for 3D point clouds.
The models studied here are designed for rotation-equivariant point-cloud classification, not time-series classification; the UCR benchmark tests robustness and sample efficiency, not whether the geometric inductive bias is optimal for these datasets.
...
\begin{itemize}
    ...
    \item Accuracy improves with training fraction for every model, though the MLP head saturates around 54\% (\cref{fig:model_data_efficiency}). XGBoost extracts substantially more signal from small training sets than the end-to-end MLP head.
...
\begin{table}[ht]
\centering
\caption{Best-per-family accuracy (\%) averaged by dataset type across the 21-dataset benchmark (oracle: best model in each family per task). ECG\,=\,\{ECG200, ECG5000\}; Image\,=\,\{MedicalImages, MiddlePhalanx variants, Plane, Phalanges, ProximalPhalanx\}; Motion\,=\,\{GunPoint, ItalyPowerDemand, GunPointOldVersusYoung, TwoLeadECG, UMD\}; Sensor\,=\,\{Sony1, Sony2, PowerCons, MPMC\}; Simulated\,=\,\{CBF, Chlorine, DistalPhalanx\}.}
...
\midrule
TFN & 88.3 & \textbf{77.7} & \textbf{89.1} & \textbf{88.2} & \textbf{76.4} \\
PointNet & 84.9 & 69.9 & 81.1 & 75.5 & 71.7 \\
DMR & \textbf{90.1} & 70.2 & 73.6 & 62.7 & 69.8 \\
\bottomrule
\end{tabular}
\end{table}

DMR leads on ECG (90.1\% vs.\ 88.3\%), where pairwise distance structure dominates, but TFN leads on all other categories.
The largest TFN advantage appears on Sensor (+25.5pp over DMR) and Motion (+15.5pp) datasets.
On Image, the families are more closely matched (70--78\%).
...
This gap indicates that the geometric inductive bias of the TFN family does not compensate for the temporal modeling advantages of convolutional and kernel-based architectures on UCR data.
The TFN models are designed for general 3D point clouds with rotation equivariance; they do not exploit the sequential structure of time-series data.

\paragraph{No hyperparameter sensitivity analysis.}
...
The central finding of this study is that rotation equivariance and classification accuracy are not in tension for small-scale point-cloud problems, but the architectural advantages of the TFN family are smaller than they appear.
Under a best-configuration protocol, the improved TFN variants lead (HybridGTTFN at 79.2\%), but at a fixed 100\% training fraction with a single classifier, the simpler DMR and PersNet models perform comparably or better (Table~\ref{tab:ucr_honest}), and no pairwise comparison survives Bonferroni correction.
Meanwhile, published time-series methods (MiniRocket: 88.9\%, ResNet: 87.3\%) substantially outperform all models in this study, indicating that the geometric inductive bias of TFNs does not compensate for the temporal modeling advantages of convolutional and kernel-based architectures on UCR data.
...
The gap between the best-accuracy (79.2\%) and fixed-configuration (70.7\%) protocols for HybridGTTFN is 8.5 points, comparable to the gap between the best and worst TFN variants.
...
On the UCR datasets, where the time-delay embedding produces point clouds whose class structure is not primarily rotation-dependent, the equivariant TFN models do not outperform simpler baselines.
...
\caption{Per-dataset test accuracy (\%) of the MLP head and the XGBoost classifier at 100\% training fraction. XGBoost generally outperforms the MLP head, though on some small datasets (e.g., CBF) the MLP head is competitive. Many MLP results are missing due to training collapse. XGBoost accuracy is the metric reported in Table~\ref{tab:ucr_main}.}
...
\caption{Mean test accuracy as a function of the training fraction, averaged over all models and datasets. XGBoost accuracy improves with more training data, while the MLP head saturates around 54\% due to the small training sets and end-to-end training instability.}
...
\caption{MLP(+aug) test accuracy (\%) at 100\% training fraction, by dataset and model. The MLP head frequently achieves below-chance accuracy (e.g., PersNet at 34\% on 3-class CBF), confirming that end-to-end training fails on small datasets. Many model--dataset pairs have no MLP results, indicating training collapse.}
