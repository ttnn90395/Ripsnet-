...
\caption{Per-model accuracy at 100\% training fraction, averaged over 21 datasets (4 trials each). Error bars are standard deviation across datasets. The improved TFN variants lead among the models in this study.}
...
\begin{itemize}
    ...
    \item Accuracy improves monotonically with training fraction for every model that trains stably, confirming that the learned descriptors capture signal rather than memorizing small training sets (\cref{fig:model_data_efficiency}).
    \item Training curves on CBF (\cref{fig:training_curves}) show that TFN variants converge more slowly than PointNet/DMR (50--100 epochs vs.\ 20) but reach comparable final accuracy.
...
\caption{XGBoost accuracy vs.\ training fraction for each model family, averaged over 21 datasets (mean $\pm$ SEM). All models improve monotonically; the TFN variants and DMR converge faster than PersNet at low fractions.}
...
\caption{Training loss (left) and accuracy (right) on CBF over 200 epochs (15 training samples). Solid lines: training set; dashed lines: validation proxy (full training set re-evaluated). TFN variants converge more slowly than PersNet and DMR but reach comparable final accuracy.}
...
\caption{Validation accuracy convergence for each model family on CBF. PersNet and DMR reach high accuracy within 20 epochs; TFN variants require 50--100 epochs but achieve similar final performance.}
...
\caption{Mean test accuracy (21 datasets) vs.\ total CPU time per forward+backward pass. Bubble size is proportional to $\sqrt{\text{parameters}}$. The TFN family occupies the high-accuracy, high-cost quadrant, while PointNet variants are fast but less accurate.}
