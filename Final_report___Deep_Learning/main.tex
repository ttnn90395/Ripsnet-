...
However, at a fixed 100\% training fraction with XGBoost, the simpler distance-matrix and PointNet baselines perform comparably (73.1\% and 73.4\%), suggesting that configuration sensitivity rather than architecture is the primary lever for these models.
\end{abstract}
...
The values in Tables~\ref{tab:ucr_main} and~\ref{tab:ucr_full} report the \emph{best accuracy} achieved by each model across all training fractions (10--100\%), trials (4), and multi-scale configurations.
...
Under a best-configuration protocol, the improved TFN variants lead (HybridGTTFN at 79.2\%), but at a fixed 100\% training fraction with XGBoost, the simpler DMR and PersNet models perform comparably or better (Table~\ref{tab:ucr_honest}), and no pairwise comparison survives Bonferroni correction.
