...
Distance-matrix and PointNet models are 50--130$\times$ faster (2--13\,ms total), reflecting their simpler $O(N)$ or $O(N^2)$ computation without equivariant tensor products.
...
\begin{itemize}
    ...
    \item Training curves on CBF (\cref{fig:training_curves}) show that GTTFNv2 requires $\sim$50 epochs to reach 80\% accuracy, while HybridGTTFN, PersNet, and DMR converge within 10--20 epochs. AttnTFN plateaus at 80\% throughout training.
    \item Timing benchmarks (\cref{fig:timing} and \cref{fig:accuracy_cost}) confirm the 50--130$\times$ cost gap between TFN and non-equivariant models, with HybridGTTFN being the most expensive at 530\,ms per pass.
\end{itemize}
...
\paragraph{Limited ablation scope.}
The isometry, density, and shape ablations (Sections~\ref{sec:isometry}, \ref{sec:density}, \ref{sec:shape_ablation}) cover only a single dataset (\texttt{CBF}) with few trials (2--4) and short training (20 epochs for shape).
The conclusions drawn---that robustness to corruption requires data augmentation rather than architectural changes, for instance---are suggestive but not statistically robust.
Extending these ablations to the full 21-dataset suite with more trials would be needed to generalize the findings.

\paragraph{Input dimensionality.}
The UCR time series are embedded into 3D point clouds via time-delay embedding, so most experiments operate in $n=3$.
The $n$-dimensional TFN generalization is verified only for $n=4$ (equivariance check), and no high-dimensional ($n>4$) benchmarks are included.
Whether the TFN family maintains its advantage in genuinely high-dimensional settings remains open.
...
The gap of 8.2 points indicates substantial sensitivity to hyperparameters.
Without a held-out validation set for configuration selection, the best-accuracy numbers should be interpreted as upper bounds rather than expected test performance.
A systematic study of which hyperparameters matter most (number of layers, hidden channels, k-NN size, radial basis width) would clarify whether this sensitivity is inherent to the architecture or an artifact of the search space.
...
The gap between the best-accuracy (79.2\%) and fixed-configuration (71.0\%) protocols for HybridGTTFN is 8.2 points, comparable to the gap between the best and worst TFN variants.
...
\caption{Training loss (left) and accuracy (right) on CBF over 200 epochs (15 training samples). Solid lines: training set; dashed lines: validation proxy. Most models converge to 100\% training accuracy; AttnTFN plateaus at 80\% with notably higher loss, suggesting instability with the small training set.}
...
\caption{Validation accuracy convergence for each model family on CBF (15 training samples). HybridGTTFN, PersNet, and DMR reach high accuracy within 10--20 epochs; GTTFNv2 requires $\sim$50 epochs to surpass 80\%; AttnTFN plateaus at 80\% throughout training.}
