#!/bin/bash
# Submit ALL remaining missing jobs with correct memory
# - No GPU on SallesInfo, all CPU
# - Big models need 32G (OnEquivariant, Attention, Relaxed, Hybrid)
# - Baselines need 16G (GTBasis, ScalarDistance*)
# - Small models need 16G
set -euo pipefail
...
submit_job() {
    ...
    sbatch --job-name="$jname" --partition=SallesInfo \
        ...
        --wrap="cd $REMOTE_DIR && python train_shape.py $ds $model $EPOCHS $t" 2>&1 | grep -q "Submitted" && submitted=$(( submitted + 1 ))
}

for ds in circles circles_noisy; do
    for model in OnEquivariantTensorFieldNetwork AttentionTensorFieldNetwork RelaxedOnEquivariantTensorFieldNetwork HybridOnEquivariantTensorFieldNetwork; do
        for t in 0 1 2; do
...
for ds in circles circles_noisy; do
    for model in GTBasisDeepSet ScalarDistanceSetTransformer; do
        for t in 0 1 2; do
            submit_job "$ds" "$model" "$t" 16G 01:00:00
        done
    done
done
...
for ds in shapes3d_topology shapes3d_geometry shapes3d_complex shapes3d_8way; do
    for model in RelaxedOnEquivariantTensorFieldNetwork HybridOnEquivariantTensorFieldNetwork; do
        for t in 0 1 2; do
            submit_job "$ds" "$model" "$t" 32G 04:00:00
        done
    done
done
